...
def data_partition(dir):
    ...
    User, UserCate, UserTime, user_traj, itemUser,UserCateNum = defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list), defaultdict(list),defaultdict(list)
    zoneItem, ItemtimeCate = defaultdict(lambda: defaultdict(int)), defaultdict(lambda: defaultdict(int))
    user_train_cate, user_train_time,user_train,user_valid,user_test,itemCate,itemPos,itemZone,cateCateName = {},{},{},{},{},{},{},{},{}
    ...
    for line in f:
        ...
        User[u].append(i)
        itemUser[i].append(u)
        UserCate[u].append(c_name)
        UserCateNum[u].append(c)
        UserTime[u].append(timestap)
        itemCate[i] = c
        cateCateName[c] = c_name
        itemPos[i] = (lan,lon)
        month, day, weekday, time_category, clock = extract_date_info(time_utc)
        user_traj[u].append([u, i, c, c_name, weekday, time_category, clock, month, day])
        itemZone[i] = zone
        zoneItem[zone][c_name] += 1
        ItemtimeCate[i][time_category] += 1
    loc_sem = defaultdict(list)
    test=defaultdict(int)
    ##计算前三多用户、前三用户访问最多类型、某地哪一时间段最多用户、某区域最多类型是哪个
    for loc in itemCate:
        #出现前三的用户
        counter = Counter(itemUser[loc])
        top_3_user = heapq.nlargest(3, counter.items(), key=lambda x: x[1])
        while len(top_3_user) < 3:
            top_3_user.append(top_3_user[-1])
        first_user = top_3_user[0][0]
        #出现前三的类型
        counter = Counter(UserCate[first_user])
        top_3_cate = heapq.nlargest(3, counter.items(), key=lambda x: x[1])
        while len(top_3_cate) < 3:
            top_3_cate.append(top_3_cate[-1])
        #时间段
        max_time = max(ItemtimeCate[loc], key=ItemtimeCate[loc].get)
        #区域类型
        max_zoneCate = max(zoneItem[itemZone[loc]], key=zoneItem[itemZone[loc]].get)

        loc_sem[loc].append([top_3_user,top_3_cate,max_time,max_zoneCate])
    for user in User:
        nfeedback = len(User[user])
        if nfeedback < 3:
            user_train[user] = User[user]
            user_train_cate[user] = UserCateNum[user]
            ...
            user_train_cate[user] = UserCateNum[user][:-2]
    ...
    return [user_train, user_valid, user_test, usernum, itemnum,catenum, user_train_cate, user_train_time,itemCate,itemPos,user_traj,loc_sem]
...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen, result_queue, SEED, user_train_cate, user_train_time,check_matrix,traj_vector_dict):
    def sample():
        ...
        sem = traj_vector_dict[user]
        return (user, seq, pos, neg, seq_cate, seq_time, pos_pop, neg_pop,sem)
...
class WarpSampler(object):
    def __init__(self, User, usernum, itemnum, user_train_cate, user_train_time, check_matrix,traj_vector_dict, batch_size=64, maxlen=10, n_workers=1):
...
def evaluatefin(model, dataset, maxlen, check_matrix, tot_pop, item_groups, epoch,traj_vector_dict, graph_adjacent_matrix, graph_node_features2, graph_adjacent_matrix2):
    [train, valid, test, usernum, itemnum, catenum, user_train_cate, user_train_time,itemCate,itemPos,user_traj,loc_sem] = dataset = copy.deepcopy(dataset)
    topk = [1, 5, 10, 20]
    NDCG = [0.0] * len(topk)
    HR = [0.0] * len(topk)
    ACC = [0.0] * len(topk)
    ...
    for u in users:
        ...
        for _ in range(100):
            ...
            item_pop1.append(check_matrix[t - 1])
        seq_pop = seq2pop(seq, tot_pop)
        predictions = -model.predict(*[np.array(l) for l in [[u], [seq], [cate], [seq_pop], item_idx, item_pop1, traj_vector_dict[u]]])
        predictions = predictions[0]
        ...
        item_idx2 = [t for t in range(1, itemnum + 1)]
        item_pop2 = [check_matrix[t - 1] for t in range(1, itemnum + 1)]
        cc = [np.array(l) for l in [[u], [seq], [cate], [seq_pop], item_idx2, item_pop2,traj_vector_dict[u]]]
        predictions2 = model.predict(*cc)
        rank_ls = torch.topk(predictions2, 20).indices
        label = test[u][0]
        valid_user += 1
...
def evaluatefindiff(model, dataset, maxlen, check_matrix, tot_pop, item_groups, epoch):
    [train, valid, test, usernum, itemnum, catenum, user_train_cate, user_train_time, itemCate,
     itemPos, user_traj] = copy.deepcopy(dataset)
    itemnum = 6449
    catenum = 285
    topk = [1, 5, 10, 20]
    NDCG = [0.0] * len(topk)
