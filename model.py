...
class GCN(torch.nn.Module):
    def __init__(self, n_input, n_hidden, n_output, dropout_rate):
        ...
        channels = [n_input] + n_hidden + [n_output]
...
class SASRec(torch.nn.Module):
    def __init__(self, user_num, item_num,cate_num, args):
        ...
        gcn_dropout = 0.3
        self.withsem = args.withsem
        self.withgraph1 = args.withgraph1
        self.withgraph2 = args.withgraph2
        self.weightsem = args.weightsem
        self.weightgraph1 = args.weightgraph1
        self.weightgraph2 = args.weightgraph2
        ...
        self.space_emb = GCN(gcn_feats_num, gcn_hidden_dim, args.hidden_units, gcn_dropout)
    ...
    def log2feats(self, log_seqs):
        ...
        return log_feats
    def log2featslb(self, log_seqs,cate_seqs,pop_seqs,sem,X,A,X2,A2):
        ...
        #融合语义特征
        if self.withsem == 1:
            sem_tensor = torch.tensor(sem).to(self.dev)
            if sem.ndim == 2:#训练128，120，50
                linear_proj = nn.Linear(sem_tensor.shape[1], self.hidden_units).to(self.dev)
                sem_tensor = linear_proj(sem_tensor)  # 变成 (128, 50)
                # 扩展维度，使其变成 (128, 120, 50)
                sem_expanded = sem_tensor.unsqueeze(1).repeat(1, self.maxlen, 1)  # (128, 120, 50)
                # 进行特征融合（相加）
            else:#预测120，50
                linear_proj = nn.Linear(sem_tensor.shape[0], self.hidden_units).to(self.dev)
                sem_tensor = linear_proj(sem_tensor)
                sem_expanded = sem_tensor.unsqueeze(0).repeat(self.maxlen, 1)
            seqs += self.weightsem * sem_expanded  # (128, 120, 50)

        #####融合图特征1
        if self.withgraph1 == 1:
            space_feats = self.space_emb(X, A)
            space_emb = torch.zeros_like(seqs)
            for i in range(len(log_seqs)):
                for j in range(len(log_seqs[i])):
                    index = log_seqs[i][j]
                    space_emb[i][j] = space_feats[index - 1]
            seqs += self.weightgraph1 * space_emb.to(self.dev)


        #####融合图特征2
        if self.withgraph2 == 1:
            space_feats2 = self.space_emb(X2, A2)
            space_emb2 = torch.zeros_like(seqs)
            for i in range(len(log_seqs)):
                for j in range(len(log_seqs[i])):
                    index = log_seqs[i][j]
                    space_emb2[i][j] = space_feats2[index - 1]
            seqs += self.weightgraph2 * space_emb2.to(self.dev)
    ...
    #lb
    def forward(self, user_ids, log_seqs, pos_seqs, neg_seqs, cate_seqs, pop_seqs,sem, graph_node_features, graph_adjacent_matrix, graph_node_features2, graph_adjacent_matrix2):  # for training
        self.X = graph_node_features
        self.A = graph_adjacent_matrix
        self.X2 = graph_node_features2
        self.A2 = graph_adjacent_matrix2
        log_feats = self.log2featslb(log_seqs,cate_seqs, pop_seqs,sem, graph_node_features, graph_adjacent_matrix, graph_node_features2, graph_adjacent_matrix2)  # user_ids hasn't been used yet
    ...
    def predict(self, user_ids, log_seqs, cate_seqs, pop_seqs, item_indices,  pop_seqs_for_ranklist, traj_vector_dict): # for inference
        # 计算seq特征
        sem = np.array(traj_vector_dict)
        log_feats = self.log2featslb(log_seqs,cate_seqs, pop_seqs,sem, self.X, self.A, self.X2,self.A2)  # user_ids hasn't been used yet
